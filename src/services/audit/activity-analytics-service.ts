...
export class ActivityAnalyticsService {
  ...
  public static async getActivityStatistics(
    ...
    try {
      ...
      if (filters.dateFrom) {
        query = query.where('created_at', '>=', dayjs(filters.dateFrom).toDate());
      }
      if (filters.dateTo) {
        query = query.where('created_at', '<=', dayjs(filters.dateTo).toDate());
      }
  ...
  public static async getDashboardData(): Promise<DashboardDataResult> {
    try {
      ...
      const fromDate = dayjs()
        .subtract(this.MAX_TREND_POINTS - 1, 'day')
        .startOf('day')
        .toDate();
    ...
    try {
      if (!startDate || !endDate) {
        return {
          ...
          message: 'startDate and endDate are mandatory',
        };
      }
